...
function parse_commandline()
    ...
    @add_arg_table s begin
        ...
        help = "Output folder"
        arg_type = String
        default = "C:/Users/cesar/Desktop/Projects/FoundationalModelsForStochastics/fimodemix/data/sdes_observations4/"
        "--skip"
...
function get_diffusion_scaling_constant(init_conditions_params, tspan, p, pode, num_paths, invalid_paths_threshold=5)
    """
    ...
    sols = get_ODE_trajectories(num_paths, init_conditions_params, tspan, pode, invalid_paths_threshold=invalid_paths_threshold)
...
function calculate_diffusion_scaling_constants(f_strs, g_strs, drift_functions, diffusion_functions, tspans, num_paths, num_samples, init_conditions_params; invalid_paths_threshold=5)
    ...
    for batch in 1:num_batches
        ...
        for i in ProgressBarBase(1:end_idx-start_idx+1)
            pode = (i, drift_functions[start_idx:end_idx])
            p = (i, drift_functions[start_idx:end_idx], diffusion_functions[start_idx:end_idx])

            idx = (batch - 1) * minibatch_size + i

            diffusion_constants = get_diffusion_scaling_constant(init_conditions_params[idx], tspans[idx], p, pode, num_paths[idx], invalid_paths_threshold)
...
function calculate_diffusion_scaling_constants_sequential(f_strs, g_strs, drift_functions, diffusion_functions, tspans, num_paths, num_samples, init_conditions_params; invalid_paths_threshold=5)
    invalid_sols = fill(false, num_samples)
    minibatch_size = min(500, num_samples)
    num_batches = ceil(Int, num_samples / minibatch_size)

    # Solve ODEs to get the diffusion constants
    solvable_ods = Dict{String, Tuple{Vector{Float32}, Vector{Float32}, Vector{Float32}}}()
    solvable_odsl = []
    ...
    for batch in 1:num_batches
        ...
        # Loop through the samples sequentially (no threading)
        for i in 1:(end_idx-start_idx+1)
            ...
            # Store valid results
            solvable_ods[f_strs[idx][1] * ";" * g_strs[idx][1]] = diffusion_constants[1:3]
            push!(solvable_odsl, f_strs[idx][1])
        end
    ...
    @info "Percentage of invalid ODE solutions in batch: $(Base.length(findall(invalid_sols))/Base.length(init_conditions_params))"
    @info "Number of solvable ODEs: $(Base.length(solvable_ods)),  $(num_samples - Base.length(findall(invalid_sols)))"
    
    return solvable_ods, invalid_sols
end
...
function solve_sdes_from_strs(f_strs, g_strs, tspans, dts, dts_eval, num_paths, num_samples, init_conditions_params; method=EM(), invalid_paths_threshold=5, hypercube_increase_percentage_for_location_grid=0.1, num_hypercube_locations_per_dim=10, evaluate_hypercube_on_regular_grid=false)
    ...
    return obs_times, obs_values, hypercube_locations, drift_functions_at_hypercube, init_conditions_params, f_strs_out, g_strs_out
end
...
function main()
    ...
    for i in start_batch:(num_batches+start_batch-1)
        ...
        store_training_data(output_path, obs_times, obs_values, hypercube_locations, drift_functions_at_hypercube, init_condition_distr_parameters_out, f_strs_out, g_strs_out, i)
        update!(job)
    end
    stop!(p_bar)
end
