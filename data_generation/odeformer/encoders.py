...
class GeneralEncoder:
    def __init__(self, params, symbols, all_operators):
        ...
        self.equation_encoder = Equation(params, symbols, self.float_encoder, all_operators, self.constant_encoder)
...
class ConstantEncoder(Encoder):
    def __init__(self, params: Namespace, force_min_max: List[int] = None):
        ...
        if force_min_max is not None:
            assert (
                force_min_max[0] < force_min_max[1]
            ...
            assert (
                -params.max_int
            ) >= self.min, f"params.min_int (= {params.min_int}) is smaller than supported constant range (self.min = {self.min})."
            assert (
                params.max_int <= self.max
