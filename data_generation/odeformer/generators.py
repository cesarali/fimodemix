...
class RandomFunctions(Generator):
    def __init__(self, params, special_words):
        ...
        self.symbols = list(self.operators) + self.constants + self.variables + ["|", "INT+", "INT-", "FLOAT+", "FLOAT-", "pow", "0"]
    ...
    def generate_float(self, rng, type="exponent"):
        sign = rng.choice([-1, 1])
        ...
        match type:
            case "exponent":
                ...
                return str(constant)
            case "uniform":
                constant = rng.uniform(-self.c_min, self.c_max)
                constant = sign * constant
                return str(constant)
            case "normal":
                constant = rng.normal(0.0, 1.0)
                constant = sign * constant
                return str(constant)
            case _:
    ...
    def generate_multi_dimensional_tree(self, rng, dimension=None, nb_unary_ops=None, nb_binary_ops=None):
        trees = []
        ...
        if nb_binary_ops is None:
            nb_binary_ops_to_use = [rng.randint(self.min_binary_ops_per_dim, self.max_binary_ops_per_dim + 1) for dim in range(dimension)]
        ...
        if nb_unary_ops is None:
            nb_unary_ops_to_use = [rng.randint(self.min_unary_ops_per_dim, self.max_unary_ops_per_dim + 1) for dim in range(dimension)]
        ...
        for i in range(dimension):
            ...
            if self.params.reduce_num_constants:
                ...
                tree = self.add_linear_transformations(rng, tree, target=self.variables, sample_type=self.sample_type)
                tree = self.add_linear_transformations(rng, tree, target=self.unaries, sample_type=self.sample_type)
            trees.append(tree)
        tree = NodeList(trees)
        ...
        for op in self.required_operators:
            if op not in tree.infix():
                return self.generate_multi_dimensional_tree(rng, dimension, nb_unary_ops, nb_binary_ops)

        return (tree, dimension, nb_unary_ops_to_use, nb_binary_ops_to_use)
    ...
    def add_prefactors(self, rng, tree):
        ...
        if transformed_prefix == tree.prefix():
            a = self.generate_float(rng, type=self.sample_type) if rng.rand() < self.params.prob_prefactor else transformed_prefix
    ...
    def _add_prefactors(self, rng, tree):
        s = str(tree.value)
        a, b, c = [self.generate_float(rng, self.sample_type) for _ in range(3)]
        ...
        if s in ["add", "sub"]:
            s += ("," if tree.children[0].value in ["add", "sub"] else mul_prefactor1) + self._add_prefactors(rng, tree.children[0])
            s += ("," if tree.children[1].value in ["add", "sub"] else mul_prefactor2) + self._add_prefactors(rng, tree.children[1])
            ...
            for c in tree.children:
                s += "," + self._add_prefactors(rng, c)
        return s

    def add_linear_transformations(self, rng, tree, target, add_after=False, sample_type="exponent"):
        ...
        offset = 0
        for idx in indices:
            a, b = self.generate_float(rng, sample_type), self.generate_float(rng, sample_type)
            if add_after:
    ...
    def generate_datapoints(self, tree, rng, dimension, n_points, n_init_conditions=1):
        ...
        if ode_integrator == "jax":
            ...
            def func(t, y, args=None):
                return jnp.concatenate(
                    [jax_tree(y, jax_param).reshape(-1, 1) for jax_tree, jax_param in zip(jax_trees, jax_params)], axis=1
                )
            ...
            try:
                trajectory = scipy.integrate.solve_ivp(
                    func, (min(times), max(times)), y0, t_eval=times, events=events
                )  # , method='RK23', rtol=1e-2, atol=1e-6)
...
if __name__ == "__main__":
    ...
    tree, _, _, _, _ = generator.generate_multi_dimensional_tree(np.random.RandomState(0), dimension=1)
    print(tree)
